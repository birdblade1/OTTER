...
//	glm::mat4 transform4 = glm::mat4(1.0f);
	...
	camera->SetPosition(glm::vec3(-1, 0.5, 3));
	camera->LookAt(glm::vec3(-0.25f));
	...
	while (!glfwWindowShouldClose(window)) {
		...
		transform3 = glm::translate(glm::mat4(1.0f), glm::vec3(-1, glm::sin(static_cast<float>(thisFrame - 3)), 0.0f));
//		transform4 = glm::rotate(glm::mat4(1.0f), static_cast<float>(thisFrame), glm::vec3(0, 0, 1));
		...
		shader->SetUniformMatrix("u_ModelViewProjection", camera->GetViewProjection() * transform3);//+ transform4
